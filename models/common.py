...
class C2f(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=0, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super(C2f, self).__init__()
        self.c = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv((2 + n) * self.c, c2, 1)  # optional act=FReLU(c2)
        self.mm = nn.ModuleList(
            Bottleneck(self.c, self.c, shortcut == 1, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))

    def forward(self, x):
        """Forward pass through C2f layer."""
        y = list(self.cv1(x).chunk(2, 1))
        y.extend(m(y[-1]) for m in self.mm)
...
class GSConv_PR(nn.Module):
    def __init__(self, in_channels, out_channels, kernnel=3, strid=1, pad=1):
        super().__init__()
        c_ = out_channels // 2
        # self.cv1 = Conv(c1, c_, k, s, None, g)
        self.SCONV = nn.Sequential(
            nn.Conv2d(in_channels, c_, kernnel, strid, pad, groups=1, bias=False),
            nn.BatchNorm2d(c_),
            # nn.GroupNorm(16,c_),
            nn.PReLU(c_),
        )

        # self.cv2 = Conv(c_, c_, k, 1, None, c_)
        self.DWCONV = nn.Sequential(
            nn.Conv2d(c_, c_, kernnel, strid, pad, groups=c_, bias=False),
            nn.BatchNorm2d(c_),
            # nn.GroupNorm(16,c_),
            nn.PReLU(c_),
        )
...
class GSModle(nn.Module):
    # GSConv https://github.com/AlanLi1997/slim-neck-by-gsconv
    def __init__(self, in_channels, out_channels, kernnel=3, strid=1, pad=1, group=1, PRelu=False):
        super().__init__()
        if PRelu:
            self.GSConv = GSConv_PR(in_channels // 2, out_channels // 2)
        else:
            self.GSConv = GSConv(in_channels // 2, out_channels // 2)
...
class GSModle1(nn.Module):
    # GSConv https://github.com/AlanLi1997/slim-neck-by-gsconv
    def __init__(self, in_channels, out_channels, PRelu=False):
        super().__init__()
        if PRelu:
            self.GSConv = GSConv_PR(in_channels // 2, out_channels // 2)
        else:
        ...
        if in_channels <= 64:
            if PRelu:
                self.double_conv = nn.Sequential(
                    GSConv_PR(in_channels, out_channels),
                    GSConv_PR(out_channels, out_channels)
                )
            else:
                self.double_conv = nn.Sequential(
                    GSConv(in_channels, out_channels),
                    GSConv(out_channels, out_channels)
                )
        else:
            self.double_conv = nn.Sequential(
                GSModle(in_channels, out_channels, PRelu),
                GSModle1(out_channels, out_channels, PRelu)
            )
...
class Down(nn.Module):
    ...
    # 下采样加上DoubleConv    DoubleConv：两次卷积加数据的归一化加Relu激活函数
    def __init__(self, in_channels, out_channels, PRelu=False):
        super().__init__()
        ...
        self.maxpool_conv = nn.Sequential(
            ...
            # nn.BatchNorm2d(in_channels),
            # nn.ReLU6(inplace=True),
            DoubleConv(in_channels, out_channels, PRelu)
        )
...
class Up(nn.Module):
    ...
    # 上采样加上DoubleConv         DoubleConv：两次卷积加数据的归一化加Relu激活函数
...
class Up_SP(nn.Module):
    """Upscaling then double conv"""

    # 上采样加上DoubleConv         DoubleConv：两次卷积加数据的归一化加Relu激活函数
    def __init__(self, in_channels, out_channels, bilinear=True):
        super().__init__()
        # self.attention = SELayer(in_channels//2)
        self.attention = StripPooling(in_channels // 2)
        # if bilinear, use the normal convolutions to reduce the number of channels
        if bilinear:
            # 双线性插值
            self.up = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
...
class StripPoolingyk(nn.Module):
    def __init__(self, in_channels, up_kwargs={'mode': 'bilinear', 'align_corners': True}):
        super(StripPoolingyk, self).__init__()
        self.pool1 = nn.AdaptiveAvgPool2d((1, None))  # 1*W
        self.pool2 = nn.AdaptiveAvgPool2d((None, 1))  # H*1
        inter_channels = int(in_channels / 4)
        self.conv1 = nn.Sequential(nn.Conv2d(in_channels, inter_channels, 1, bias=False),
    ...
    def forward(self, x):
        ...
        # x3 = F.interpolate(self.conv3(self.pool2(x1)), (h, w), **self._up_kwargs)#结构图的H*1的部分
        # x4 = self.conv4(F.relu_(x2 + x3))#结合1*W和H*1的特征
        x4 = self.conv4(F.relu_(x2))  # 结合1*W和H*1的特征
        out = self.conv5(x4)
        return F.relu_(x + out)  # 将输出的特征与原始输入特征结合
