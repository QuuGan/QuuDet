...
class ComputeLoss:
    # Compute losses
    def __init__(self, model, autobalance=False, out_layer=-1):
        ...
        det = model.module.model[out_layer] if is_parallel(model) else model.model[out_layer]  # Detect() module
...
class ComputeLossOTA:
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        self.device = device = targets.device
        lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
...
class v8DetectionLoss:
    ...
    def __init__(self, model, out_layer=-1):  # model must be de-paralleled

        device = next(model.parameters()).device  # get model device

        m = model.model[out_layer]  # Detect() module
    ...
    def preprocess(self, targets, batch_size, scale_tensor):
        ...
        if targets.shape[0] == 0:
            ...
            out[..., 1:5] = metrics.xywh2xyxy(out[..., 1:5].mul_(scale_tensor))
        return out
    ...
    def __call__(self, preds, targets):
        ...
        target_scores_sum = max(target_scores.sum(), torch.ones(1).squeeze())
        ...
        loss[1] *= 0.5  # cls gain
        loss[2] *= 1.5  # dfl gain
        loss_total = torch.tensor([loss[0] + loss[1] + loss[2]]).to(self.device)
        return loss.sum() * batch_size, torch.cat((loss, loss_total)).detach()


class ComputeLossYOLOv1:
    def __init__(self, model, noobj_weight=0.5):
        super(ComputeLossYOLOv1, self).__init__()
    ...
    def __call__(self, pred, target):
        """
        :param pred:YOLO  (batch_size,30,7,7)
        :param label:Dataset (batch_size,30,7,7)大小的label
        :return:yolo_loss(noobj_loss+obj_loss+classes_loss+confidence_loss)
        """
        batch_size = pred.shape[0]
        label = self.build_label(target, self.nc, batch_size)
        # 开始遍历每一个grid
        grid_size = 1 / 7.0
            ...
            for i in range(7):  # x
                for j in range(7):  # y
                    ...
                    if label_grid[4] == 1:
                        ...
                        iou1 = self.calculate_iou(bbox1_pred, bbox_label)
                        iou2 = self.calculate_iou(bbox2_pred, bbox_label)
        ...
        noobj_loss *= self.noobj_weight
        classes_loss += confidence_loss
        loss = obj_loss + noobj_loss + classes_loss
        return loss, torch.cat((torch.tensor(noobj_loss).unsqueeze(0), torch.tensor(obj_loss).unsqueeze(0),
                                torch.tensor(classes_loss).unsqueeze(0), loss.unsqueeze(0))).detach()
    ...
    def convert_bbox2labels(self, bboxes, nc):
        grid_size = 1 / 7.0
        label = np.zeros(shape=(nc + 10, 7, 7))
        for i in range(len(bboxes)):
            ...
            label[0:5, grid_x, grid_y] = torch.tensor([cx, cy, bbox[3], bbox[4], 1])
            label[5:10, grid_x, grid_y] = torch.tensor([cx, cy, bbox[3], bbox[4], 1])
            label[10 + int(bbox[0]), grid_x, grid_y] = 1
        return label

    def build_label(self, target, nc, batch_size):
        idx_array = np.arange(batch_size)
        label = torch.zeros((batch_size, nc + 10, 7, 7)).to(self.device)
        for idx in idx_array:
            box = target[target[:, 0] == idx][:, 1:]
            label[idx] = torch.tensor(self.convert_bbox2labels(box, nc))
        return label
