...
def compute_mIoU(gt_dir, pred_dir, png_name_list, num_classes, name_classes):
    ...
    for ind in range(len(gt_imgs)):
        ...
        # If the size of the image segmentation result is different from the label, this image will not be calculated
        if len(label.flatten()) != len(pred.flatten()):
            s = 'Skipping: len(gt) = {:d}, len(pred) = {:d}, {:s}, {:s}'.format(
        ...
        # ------------------------------------------------#
        #   Calculating a Picture 21 × Hist matrix of 21 and accumulate
        # ------------------------------------------------#
        ...
        if ind > 0 and ind % 10 == 0:
            s = '{:d} / {:d}: mIou-{:0.2f}%; mPA-{:0.2f}%; Accuracy-{:0.2f}%'.format(
                ind,
                len(gt_imgs),
    ...
    for ind_class in range(num_classes):
        s = '===>' + name_classes[ind_class] + ':\tIou-' + str(round(IoUs[ind_class] * 100, 2)) \
              + '; Recall (equal to the PA)-' + str(round(PA_Recall[ind_class] * 100, 2)) + '; Precision-' + str(
            round(Precision[ind_class] * 100, 2))
        txt_list.append(s)

    # -----------------------------------------------------------------#
    #   Calculate the average mIoU value for all categories on all validation set images, ignoring NaN values during calculation
    # -----------------------------------------------------------------#
    s = '===> mIoU: ' + str(round(np.nanmean(IoUs) * 100, 2)) + '; mPA: ' + str(
        round(np.nanmean(PA_Recall) * 100, 2)) + '; Accuracy: ' + str(round(per_Accuracy(hist) * 100, 2))
    txt_list.append(s)
    return np.array(hist, np.int), IoUs, PA_Recall, Precision,txt_list
